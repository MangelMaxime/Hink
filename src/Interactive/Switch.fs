...
module Switch =
    ...
    and Switch(?x, ?y, ?width, ?height, ?theme, ?onStateChange : SwitchStateChange -> unit) as self =
        ...
        let drawBackground color =
            background.clear().beginFill(float color)
                      .drawRoundedRect(0., 0., theme.Width, theme.Height, theme.Height / 2.).endFill() |> ignore
        let circle = Graphics()

        let drawCircle color =
            let x =
                if active then theme.Width - (theme.CircleRadius + theme.CirclePadding)
                else theme.CircleRadius + theme.CirclePadding
            circle.clear().beginFill(float color).drawCircle(x, theme.Height / 2., theme.CircleRadius).endFill()
            |> ignore
