...
module Core =
    ...
    and IClickable<'T> =
        abstract OnClick : IEvent<'T> with get

    // Init the renderer
    // Init the canvas
    and IStateChangeable<'T> =
        abstract OnStateChange : IEvent<'T> with get
    ...
    and Application() =
        ...
        member val Widgets = [] with get, set
        ...
        member self.Init() =
            ...
            self.Canvas.addEventListener_click (fun ev ->
                self.Canvas.focus()
                null)
            document.body.appendChild (self.Canvas) |> ignore
        ...
        member self.RequestUpdate() = window.requestAnimationFrame (fun dt -> self.Update(dt)) |> ignore

        member self.Update(dt : float) =
            self.Renderer.render (self.RootContainer)
            self.RequestUpdate()

        member self.AddWidget(widget : Widget) = self.RootContainer.addChild (widget.UI) |> ignore
